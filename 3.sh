...
echo $'\E[31m' "______________________________________________________";
echo -n $'\E[33m' "Creando usuario ..."; adduser --disabled-password --gecos "" sinusbot4 > /dev/null; echo $'\E[32m' " Completado.";
echo $'\E[31m' "______________________________________________________";
echo -n $'\E[33m' "Creando directorio ..."; mkdir -p /opt/ts3soundboard4 > /dev/null; echo $'\E[32m' " Completado.";
echo $'\E[31m' "______________________________________________________";
echo -n $'\E[33m' "Concediendo permisos directorio ..."; chown -R sinusbot4:sinusbot4 /opt/ts3soundboard4 > /dev/null; echo $'\E[32m' " Completado.";

cd /opt/ts3soundboard4 > /dev/null;
...
cd /opt/ts3soundboard4/scripts > /dev/null;
...
chown -R sinusbot4:sinusbot4 /opt/ts3soundboard4 > /dev/null;

cd /opt/ts3soundboard4 > /dev/null;
rm /tmp/.sinusbot.lock > /dev/null;
rm /tmp/.X11-unix/X40 > /dev/null;
echo -n $'\E[33m' "Activando sinusbot ..."; screen -dmS sinusbot4 bash -c 'su sinusbot4 -c "./sinusbot --override-password=jp7237056"' > /dev/null; echo $'\E[32m' " Completado.";
echo $'\E[31m' "______________________________________________________";
echo -n $'\E[35m'
cat << "EOF"
