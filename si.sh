...
apt-get install -y x11vnc xvfb libxcursor1 ca-certificates curl bzip2 libnss3 libegl1-mesa x11-xkb-utils libasound2 libpci3 libxslt1.1 libxkbcommon0 libxss1 libxcomposite1 libglib2.0-0 python nano screen zip > /dev/null;
...
echo -n $'\E[33m' "Descargando TS3 ..."; wget -q http://dl.4players.de/ts/releases/3.5.3/TeamSpeak3-Client-linux_amd64-3.5.3.run > /dev/null; echo $'\E[32m' " Completado.";

chmod 0755 TeamSpeak3-Client-linux_amd64-3.5.3.run > /dev/null;
echo $'\E[30m' "______________________________________________________";
echo -n $'\E[31m' "Accion requerida!, presione(Q, Enter, Y, Enter)"; echo $'\E[30m' " Completado."; ./TeamSpeak3-Client-linux_amd64-3.5.3.run > /dev/null;
echo $'\E[30m' "....";
