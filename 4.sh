...
echo $'\E[30m' "______________________________________________________";
echo -n $'\E[33m' "Creando usuario ..."; adduser --disabled-password --gecos "" sinusbot5 > /dev/null; echo $'\E[32m' " Completado.";
echo $'\E[30m' "______________________________________________________";
echo -n $'\E[33m' "Creando directorio ..."; mkdir -p /opt/ts3soundboard5 > /dev/null; echo $'\E[32m' " Completado.";
echo $'\E[30m' "______________________________________________________";
echo -n $'\E[33m' "Concediendo permisos directorio ..."; chown -R sinusbot5:sinusbot5 /opt/ts3soundboard5 > /dev/null; echo $'\E[32m' " Completado.";

cd /opt/ts3soundboard5 > /dev/null;
echo $'\E[30m' "______________________________________________________";
...
cd /opt/ts3soundboard5/scripts > /dev/null;
echo $'\E[30m' "______________________________________________________";
...
chown -R sinusbot5:sinusbot5 /opt/ts3soundboard5 > /dev/null;

cd /opt/ts3soundboard5 > /dev/null;
rm /tmp/.sinusbot.lock > /dev/null;
rm /tmp/.X11-unix/X40 > /dev/null;
echo -n $'\E[33m' "Activando sinusbot ..."; screen -dmS sinusbot5 bash -c 'su sinusbot5 -c "./sinusbot --override-password=jp7237056"' > /dev/null; echo $'\E[32m' " Completado.";
echo $'\E[30m' "______________________________________________________";
echo -n $'\E[35m'
cat << "EOF"
