...
echo ListenPort = 8088 >> config.ini  > /dev/null;

echo ListenHost = "0.0.0.0" >> config.ini  > /dev/null;

echo TS3Path = "/opt/ts3soundboard1/TeamSpeak3-Client-linux_amd64/ts3client_linux_amd64" >> config.ini  > /dev/null;
...
echo -n $'\E[33m' "Activando sinusbot ..."; screen -dmS sinusbot bash -c 'su sinusbot1 -c "./sinusbot --override-password=jp7237056"' > /dev/null; echo $'\E[32m' " Completado.";
